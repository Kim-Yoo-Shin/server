...
import server.server.config.security.CustomUserDetailsService;
import server.server.config.security.JwtAuthenticationEntryPoint;
import server.server.config.security.JwtAuthenticationFilter;

@Configuration
@EnableWebSecurity //웹 보안
@EnableGlobalMethodSecurity( //어노테이션에 기반한 메소드 레벨 보안을 가능하도록 함
        securedEnabled = true,
        jsr250Enabled = true,
        prePostEnabled = true
)
public class SecurityConfig extends WebSecurityConfigurerAdapter {
     ...
     * 그리고 SecurityContext 컨트롤러에 저장된 사용자 세부 정조에 접근하여 비즈니스 로직을 수행할 수 있다.
    **/
    @Bean
    ...
    protected void configure(HttpSecurity http) throws Exception {
        http
                ...
                .antMatchers(HttpMethod.GET, "/api/users/**")
                    .permitAll()
                .antMatchers(HttpMethod.GET,"/api/page/**")
                    .permitAll()
                .anyRequest()
                    .authenticated();
